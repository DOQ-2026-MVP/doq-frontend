...
import type { IngestionDetail } from "./types"

export const postUpload = (file: File) => {
    const form = new FormData()
    form.append("file", file)
    return ApiHelper.post(API_PATH.INGESTION.UPLOADS(), form, { headers: { "Content-Type": "multipart/form-data" } })
}

export const postUploadFor = (ingestionId: number | string, file: File) => {
    const form = new FormData()
    form.append("file", file)
    return ApiHelper.post(API_PATH.INGESTION.UPLOADS_FOR(ingestionId), form, {
...
export const deleteUpload = (ingestionId: number | string, uploadId: number | string) =>
    ApiHelper.delete(API_PATH.INGESTION.UPLOAD_DELETE(ingestionId, uploadId))

export const postRecords = (ingestionId: number | string, body: unknown[]) =>
    ApiHelper.post(API_PATH.INGESTION.RECORDS_FOR(ingestionId), body)

export const getRecordsFor = (ingestionId: number | string) =>
    ApiHelper.get(API_PATH.INGESTION.RECORDS_FOR(ingestionId))

export const putRecord = (ingestionId: number | string, recordId: number | string, body: unknown) =>
    ApiHelper.put(API_PATH.INGESTION.RECORD_PUT(ingestionId, recordId), body)

export const deleteRecordsAll = (ingestionId: number | string) =>
    ApiHelper.delete(API_PATH.INGESTION.RECORDS_DELETE_ALL(ingestionId))

export const getIngestionDetail = (ingestionId: number | string) =>
    ApiHelper.get<IngestionDetail>(API_PATH.INGESTION.INGESTION_DETAIL(ingestionId))

// Global records endpoint (no ingestionId in path)
export const postRecordsGlobal = (body: unknown[]) => ApiHelper.post(API_PATH.INGESTION.RECORDS_GLOBAL(), body)

export const deleteRecord = (ingestionId: number | string, recordId: number | string) =>
    ApiHelper.delete(API_PATH.INGESTION.RECORD_DELETE(ingestionId, recordId))
